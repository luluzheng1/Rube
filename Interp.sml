...
fun eval p (A, H, ENil) = (A, H, RNIL)
  ...
  | eval p (A, H, EFldWr (s, e)) = 
    let val option_location = lookup "self" A
        ...
        val rv = rubeval (A', H', v)
        val () = update fs (s, rv)
    in (A', H', rv)
    end 

  | eval p (A, H, EIf (e1, e2, e3)) =
    let val (A', H', guard) = eval p (A, H, e1)
        val evaluate =
          (case guard
  ...
  | eval p (A, H, ESeq (e1, e2)) =
    let val (A', H', v1) = eval p (A, H, e1)
        val (A'', H'', v2) = eval p (A', H', e2)
    in (A'', H'', v2)
    end
...
fun run (p:rube_prog):string = 
    let val {prog_clss=cls, prog_main=e} = p
        ...
        fun stringify(RNIL: rubevalue):string = "nil"
          | stringify(RINT n) = Int.toString n
          | stringify(RSTR s) = s
          | stringify _ = raise Fail "Not implemented"
